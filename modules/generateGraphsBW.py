...
def generateIndivGraph(colorVal):
    ...
    plt.xlabel('Vdio')
    plt.ylabel('I')
    plt.scatter(colorDf['Vdio'], colorDf['I'], c='black', cmap=plt.cm.Blues, edgecolors='none')
    ...
    plt.plot(x, objective(x, *popt), 'r-', label=colorVal.capitalize()+ ' LED Diode Regression', color='black', linewidth=.5)
    # Add a legend with white text.
    plt.legend(loc='upper left', facecolor='white', edgecolor='white', frameon=True)
    plt.savefig('outputs/' + colorVal + 'BW.png')
    plt.show()
